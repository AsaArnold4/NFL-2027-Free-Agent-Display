...
def render_fa_table(df: pd.DataFrame) -> str:
    ...
    if df.empty:
        return ('<div style="padding:40px;text-align:center;color:var(--ink-2)">'
    ...
    for _, r in df.iterrows():
        ...
        rep_html = (f'<span class="empty-rep">—</span>'
                    if r["agency_name"] == "—"
                    else ag_esc)
        ...
        <tr>
          ...
          <td class="player-cell">
            <a class="player-name" href="?player={slug}">{name_esc}</a>
            <div><span class="fa-pill">{_html.escape(str(r['fa_type']))}</span></div>
          </td>
          <td><span class="pos-pill">{pos_esc}</span></td>
          <td class="team-cell">{team_esc}</td>
          <td class="age-cell">{int(r['age'])}</td>
          <td class="snap-cell">{r['snap_pct']:.0f}%</td>
          <td><span class="tier-pill {tier_class}">{tier_str}</span></td>
          <td class="prior-apy">${r['prior_apy']/1e6:.1f}M</td>
          <td>
            <div class="projection">{proj_str}</div>
            <div class="comps">{comps_str}</div>
          </td>
          <td class="rep-cell">{rep_html}</td>
        </tr>""")
...
# =============================================================================
with tab_browse:
    # Check for clicked player (URL routing)
    ...
    sort_map = {
        ...
        "2027 Projection midpoint (high → low)": (None,      False),  # custom
    ...
    # Clamp if filters shrink the list
    ...
    st.markdown(render_fa_table(page_slice), unsafe_allow_html=True)
    ...
    if selected_slug:
        ...
        if len(matches):
            ...
            with back_r:
                st.markdown(
                    f"<div style='padding-top:6px;color:var(--ink-2)'>"
...
with tab_top:
    ...
    for i, pos in enumerate(positions_present):
        ...
        with cols[i % 2]:
            ...
            st.markdown(render_fa_table(block), unsafe_allow_html=True)
            st.write("")
            ...
            with col:
                st.markdown(
                    ...
                    <div class="detail-card">
                      <div class="name">{_html.escape(str(r['player_name']))}</div>
                      <div style="margin-top:6px">
                        <span class="tag tag-meta">{_html.escape(str(r['position']))}</span>
                        <span class="tag tag-meta">{_html.escape(str(r['team']))}</span>
                        <span class="tag tag-age">Age {int(r['age'])}</span>
                        <span class="tag tag-fa">{_html.escape(str(r['fa_type']))}</span>
                      </div>
                      <hr/>
                      <div class="label">Prior APY</div>
                      <div class="val" style="font-size:22px">${r['prior_apy']/1e6:.1f}M</div>
                      <div class="label" style="margin-top:10px">2027 Projection</div>
                      <div class="proj-band" style="font-size:22px">{proj_str}</div>
                      <div style="color:var(--ink-3);font-size:12px;margin-top:2px">
                        {int(r['comp_count'])} comps
                      </div>
                      <div class="label" style="margin-top:10px">Tier</div>
                      <div><span class="tier-pill {tier_cls}">{tier_str}</span></div>
                      <div class="label" style="margin-top:10px">Snap %</div>
                      <div class="val">{r['snap_pct']:.0f}%</div>
                      <div class="label" style="margin-top:10px">Representation</div>
                      <div class="val" style="font-weight:500">
                        {_html.escape(str(r['agency_name']))}
                      </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
